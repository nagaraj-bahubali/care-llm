...
def generate_text(state: AgentState):
    ...
    sys_msg = prompts["v1"]["generator_agent"]["diagnosis"]
...
def simplify_text(state: AgentState):
    ...
    sys_msg = prompts["v1"]["simplifier_agent"]["diagnosis"]
    prompt = [
        SystemMessage(content=sys_msg),
...
async def summarize(medical_text: str, user_profile: dict):

    input_message = {"medical_text": medical_text,
                     "user_profile": user_profile, "repeat_counter": 0}
